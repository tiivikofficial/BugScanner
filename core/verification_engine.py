...
This layer evaluates evidence already collected by scanners. It never expands
scope, bypasses controls, or executes exploitation.
"""
...
class VerificationEngine:
    ...
    def verify(cls, finding: Vulnerability, peer_findings: list[Vulnerability] | None = None) -> VerificationResult:
        ...
        corroborating = {
            (p.vuln_type.lower(), (p.parameter or "").lower(), (p.method or "GET").upper(), p.evidence[:160])
            for p in peers if p is not finding
        }
        confirmations = min(2, len(corroborating))
        score = round(min(1.0, score + confirmations * 0.10), 2)
        status = next(status for threshold, status in cls._THRESHOLDS if score >= threshold)
        return VerificationResult(score, status, tuple(signals), confirmations)
    ...
    def enrich(cls, findings: list[Vulnerability]) -> list[Vulnerability]:
        ...
        for group in groups.values():
            for finding in group:
                ...
                finding.independent_confirmations = max(finding.independent_confirmations, result.independent_confirmations)
                if result.signals:
