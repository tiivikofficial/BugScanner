...
class AdaptiveRateLimiter:
    ...
    def __init__(self, default_rps: float = 10.0, min_rps: float = 1.0,
        ...
        # Older configs used a reduction factor (e.g. 0.5). Newer configs use
        # a divisor (e.g. 2). Preserve both meanings without weakening limits.
        if 0 < backoff_multiplier < 1:
            backoff_multiplier = 1.0 / backoff_multiplier
    ...
    async def acquire(self, domain: str):
        if domain in self._paused_until:
            ...
            if remaining > 0:
                console.print(f"[yellow]⏸  {domain} — {remaining:.0f}s back-pressure pause[/yellow]")
                await asyncio.sleep(remaining)
            self._paused_until.pop(domain, None)
        await self._get_bucket(domain).acquire()

    def on_response(self, domain: str, status_code: int, retry_after: float | None = None):
        ...
        if status_code == 429:
            ...
            if pause:
                self._paused_until[domain] = max(self._paused_until.get(domain, 0.0), time.monotonic() + pause)
            console.print(f"[red]⚡ 429 — {domain} RPS → {new_rps:.1f}{f', pause {pause:.0f}s' if pause else ''}[/red]")
        elif status_code == 503:
            pause = max(float(self.pause_on_503), retry_after or 0.0)
            self._paused_until[domain] = max(self._paused_until.get(domain, 0.0), time.monotonic() + pause)
            self._success_streak[domain] = 0
            console.print(f"[red]🛑 503 — {domain} paused for {pause:.0f}s[/red]")
            ...
            if self._success_streak[domain] % 20 == 0:
                ...
                if new_rps > bucket.rps:
                    bucket.rps = new_rps
                    console.print(f"[green]📈 {domain} RPS → {new_rps:.1f}[/green]")

    def get_stats(self) -> dict:
        return {domain: {"current_rps": round(bucket.rps, 2), "success_streak": self._success_streak.get(domain, 0), "paused": self._paused_until.get(domain, 0) > time.monotonic()} for domain, bucket in self._buckets.items()}
